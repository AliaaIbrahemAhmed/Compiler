...
pair<Node, Node>
NFA::unionOP(const vector<pair<Node, Node>> &nodes, bool isEndState, int priority, string patternName) {
    Node s = *new Node({*new State(false, 0, generateNewStateName())});
    Node e = *new Node({*new State(isEndState, priority, patternName)});
    TRANSITIONS sTransitions, eTransitions;
...
vector<pair<Node, Node>> NFA::generateNFAforKWandP(const set<string> &rules, bool which) {
    ...
    for (string p: rules) {
        ...
        for (int i = 0; i < p.size() - 1; i++) {
            ...
            transitions.insert({convertToString(p[i]), newNode});
...
pair<Node, Node> NFA::singleNodeNfa(const string &string1, bool isEndState, int priority, string patternNAme) {
    if (string1 == EPSILON) {
        Node s = *new Node({*new State(false, 0, generateNewStateName())});
        ...
        Node b;
        if (string1.size() == 1) b = *new Node({*new State(isEndState, priority, patternNAme)});
        else b = *new Node({*new State(false, 0, generateNewStateName())});
        sTransitions.insert({convertToString(string1[0]), b});
        ...
        for (int i = 1; i < string1.size() - 1; i++) {
            ...
            nfa[prevNode].insert({convertToString(string1[i]), newNode});
            prevNode = newNode;
        }
        if (string1.size() > 1) {
            Node newNode = *new Node({*new State(isEndState, priority, patternNAme)});
...
pair<Node, Node> NFA::range(const string &expression, bool isEndState, int priority, string patternName) {
    vector<string> parsed = customSplit(expression, '-');
    char start = parsed[0][0];
    char end = parsed[1][0];
    vector<pair<Node, Node>> nodes;
    for (char currentChar = start; currentChar <= end; ++currentChar) {
        nodes.push_back(singleNodeNfa(convertToString(currentChar), false, 0, generateNewStateName()));
    }
...
pair<Node, Node> NFA::parseAND(const string &s, bool isEndState, int priority, string patternName) {
    if (isRange(s))return range(s, isEndState, priority, patternName);
    vector<string> parsed = customSplit(s, ' ');
    vector<pair<Node, Node>> ns = *new vector<pair<Node, Node>>;
    for (auto i: parsed) {
        if (isRange(s)) ns.push_back(range(s, isEndState, priority, patternName));
            ...
            ns.push_back(singleNodeNfa(i, false, 0,generateNewStateName()));
        } else if (i.size() > 1 && i.at(i.size() - 1) == '+') {
            ns.push_back(positiveClosure(singleNodeNfa(i.substr(0, i.size() - 1), false, 0,generateNewStateName()), false, 0));
        } else if (i.size() > 1 && i.at(i.size() - 1) == '*') {
            ns.push_back(kleeneClosure(singleNodeNfa(i.substr(0, i.size() - 1), false, 0,generateNewStateName()), false, 0));
        } else if (!i.empty()) {
            ns.push_back(singleNodeNfa(i, false, 0,generateNewStateName()));
        }
    }
        ...
        if (c == '(' && !escapeMood) {
            ...
            if (s.top() == "(") s.pop();
                ...
                while (!s.empty() && s.top() != "(") {
                    ...
                    string topExp = s.top();
                    s.pop();
                    lastRes = eatBackward(topExp, lastRes, nodes, false, 0);
                    nodes.push(lastRes);
                }
            }
